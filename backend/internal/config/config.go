...
type GatewayConfig struct {
	...
	// DeviceIdleTimeoutMinutes: 设备空闲超时时间（分钟），默认 5 分钟。
	// 用于 Anthropic OAuth/SetupToken 账号的活跃设备数限制（粘性 hash
	// 作为设备 key）。空闲超过此时间的设备名额自动释放。
	DeviceIdleTimeoutMinutes int `mapstructure:"device_idle_timeout_minutes"`
