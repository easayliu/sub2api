...
// envBlockSentinel marks the Claude Code system-prompt env block. Used by the
// inbound validator (claude_code_validator.go) to locate the env preamble.
...
// provide a header. Client-sent X-Stainless-* values pass through verbatim;
// these defaults only fill in missing fields.
...
func (s *IdentityService) createFingerprintFromHeaders(headers http.Header) *Fingerprint {
	...
	// All fields including OS/Arch are sourced from the client so the
	// fingerprint reflects the real client platform; rewriting is no-op.
