<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="form.platform === 'anthropic' && accountCategory === 'aws-anthropic'"
...
const createAccountAndFinish = async (
  ...
  // RPM 限制对 aws-anthropic 账号也生效（OAuth/SetupToken 的 RPM 已在各自分支注入 extra）
  if (platform === 'anthropic' && type === 'aws-anthropic' && rpmLimitEnabled.value) {
...
const handleAnthropicExchange = async (authCode: string) => {
  ...
  try {
    ...
    if (deviceLimitEnabled.value && maxDevices.value != null && maxDevices.value > 0) {
      extra.max_devices = maxDevices.value
      extra.device_idle_timeout_minutes = deviceIdleTimeout.value ?? 5
    }
