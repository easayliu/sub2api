...
/**
 * Get capacity summary (concurrency/devices/RPM) for all active groups
 */
export async function getCapacitySummary(): Promise<
  { group_id: number; concurrency_used: number; concurrency_max: number; devices_used: number; devices_max: number; rpm_used: number; rpm_max: number }[]
> {
  const { data } = await apiClient.get<
    { group_id: number; concurrency_used: number; concurrency_max: number; devices_used: number; devices_max: number; rpm_used: number; rpm_max: number }[]
